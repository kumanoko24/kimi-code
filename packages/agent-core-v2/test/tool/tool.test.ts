...
function secondaryAndProfileModelFlags(): TestAgentServiceOverride {
  return appService(
    IFlagService,
    stubFlag(
      (id) =>
        id === SECONDARY_MODEL_FLAG_ID || id === AGENT_PROFILE_MODEL_BINDING_FLAG_ID,
    ),
  );
}
...
describe('Agent tool execution contract', () => {
  ...
  it('lets an explicit pool choice override the target profile exact binding', async () => {
    const lifecycle = createAgentLifecycleStub({ createAgentIds: ['agent-child'] });
    const context = createAgentToolContext(
      lifecycle,
      sessionService(
      ...
      secondaryAndProfileModelFlags(),
      {
        initialConfig: {
          secondaryModel: {
            defaultModel: 'provider/fast',
            models: { 'provider/fast': 'fast', 'provider/smart': 'hard tasks' },
          },
        },
      },
    );
    ...
    expect(lifecycle.create).toHaveBeenCalledWith(
      expect.objectContaining({
        binding: expect.objectContaining({ model: 'provider/smart', thinking: undefined }),
      }),
    );
  });

  it('maps a primary target-profile preference to the caller model', async () => {
    const lifecycle = createAgentLifecycleStub({ createAgentIds: ['agent-child'] });
    const context = createAgentToolContext(
      lifecycle,
      sessionService(
