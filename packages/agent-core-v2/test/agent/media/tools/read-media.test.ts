...
describe("AgentMediaToolsRegistrar", () => {
  ...
  function createRegistrarHarness(
    options: { readonly videoFallback?: boolean } = {},
  ) {
    const registry = new AgentToolRegistryService();
    ...
    const runtime: IAgentRuntimeService = {
      ...
      isAvailable: (required = []) =>
        runtimeAvailable && baseRuntime.isAvailable(required),
    ...
    const registrar = new AgentMediaToolsRegistrar(
      ...
      {
        enabled: () => options.videoFallback === true,
  ...
  it("wires the provider-configured fallback only when its experimental flag is enabled", () => {
    ...
    expect(
      (disabled.registry.resolve("ReadMediaFile") as ReadMediaFileTool)
        .description,
    ).toContain("Video files are not supported");
    ...
    expect(
      (enabled.registry.resolve("ReadMediaFile") as ReadMediaFileTool)
        .description,
    ).toContain("configured fallback model");
  });
  ...
  it("combines model media support with runtime filesystem availability", () => {
    const { registry, bindModel, setRuntimeAvailable } =
...
describe("createVideoAnalyzer", () => {
  it("uploads the video and asks the fallback model with the configured effort", async () => {
    ...
    const request = vi.fn(async function* () {
      const message: Message = {
        role: "assistant",
        content: [
          { type: "text", text: "A blue square moves from left to right." },
        ],
        toolCalls: [],
      };
      ...
      expect.objectContaining({
        ...
        messages: [
          expect.objectContaining({
            role: "user",
            content: [
              { type: "text", text: "How does the blue square move?" },
              uploaded,
            ],
          }),
        ],
  ...
  it("returns undefined when the fallback requester cannot upload video", () => {
    expect(
      createVideoAnalyzer(undefined, "kimi-code/k3", "max"),
    ).toBeUndefined();
    expect(
      createVideoAnalyzer(
        { request: vi.fn() } as unknown as ModelRequester,
        "kimi-code/k3",
        "max",
      ),
    ).toBeUndefined();
