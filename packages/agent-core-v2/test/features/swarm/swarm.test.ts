...
describe('AgentSwarmTool', () => {
  ...
  it('resolves every new swarm task from an exact target-profile binding', async () => {
    ...
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(DEFAULT_CALLER_PROFILE, [reviewer]), stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' }));
