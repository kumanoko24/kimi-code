...
describe('AgentSwarmTool', () => {
  ...
  it('resolves every new swarm task from an exact target-profile binding', async () => {
    ...
    const callerProfile = stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' });
    const catalog = stubSwarmCatalog(DEFAULT_CALLER_PROFILE, [reviewer]);
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), config, stubFlag(true), realSubagents(catalog, config, callerProfile), callerProfile);
    ...
    expect(host.swarmService.run).toHaveBeenCalledWith(
      expect.objectContaining({
        tasks: [
          expect.objectContaining({
            plan: { profileName: 'gpt-reviewer', model: 'provider/reviewer', modelSource: 'profile_binding', thinking: 'xhigh', fork: false },
          }),
          expect.objectContaining({
            plan: { profileName: 'gpt-reviewer', model: 'provider/reviewer', modelSource: 'profile_binding', thinking: 'xhigh', fork: false },
          }),
