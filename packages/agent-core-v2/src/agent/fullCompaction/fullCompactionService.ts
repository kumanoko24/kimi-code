...
export class AgentFullCompactionService extends Service implements IAgentFullCompactionService {
  ...
  private async tryNativeCompaction(input: {
    ...
    const messages = stripDynamicToolContext(input.originalHistory).filter(
      (message) => !isContextBudgetReminder(message),
    );
    ...
    if (effectiveMaxContextTokens > 0 && estimatedInputTokens > effectiveMaxContextTokens) {
    ...
    const measuredNativeTokens =
      compacted.usage.output > 0
        ? compacted.usage.output + this.tokenCounting.estimateMessages(appended)
        : undefined;
    const wireLines = await this.captureWireLines();
    const result = this.context.applyCompaction({
    ...
    const properties: CompactionFinishedEvent = {
      ...
      ...aheadReminderTelemetry(input.originalHistory),
    };
    this.telemetry.track2('compaction_finished', properties);
    return result;
  }
