...
export const telemetryEventDefinitions = {
  ...
  subagent_created: defineTelemetryEvent<SubagentCreatedEvent>({
    ...
    properties: {
      ...
      model_source:
        "How the bound model was chosen: 'forced' = [secondary_model].force, 'primary_override' = explicit \"primary\" request, 'profile_binding' = exact agent-file binding, 'inherited' = caller's own model (no pool or fork), 'secondary_pool' = [secondary_model.models] pool pick; omitted when no binding resolution happened (e.g. resume)",
    },
  }),
