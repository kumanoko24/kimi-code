...
vi.mock(
  "@moonshot-ai/agent-core-v2/_base/execEnv/environmentProbe",
...
describe("SDKRpcClientV2 (agent-core-v2 wiring)", () => {
  it("reads OAuth credentials from a separate auth home in both SDK and engine paths", async () => {
    ...
    await new FileTokenStorage(join(authHomeDir, "credentials")).save(
      "kimi-code",
      token,
    );
    ...
    try {
      await expect(client.auth.getCachedAccessToken()).resolves.toBe(
        "shared-access-token",
      );
      await expect(
  ...
  it("creates sessions in the separate primary session home", async () => {
    const homeDir = await mkdtemp(join(tmpdir(), "kimi-sdk-v2-"));
    const sessionHomeDir = await mkdtemp(
      join(tmpdir(), "kimi-sdk-v2-session-"),
    );
    ...
    try {
      ...
      expect(
        summary?.sessionDir.startsWith(join(sessionHomeDir, "sessions")),
      ).toBe(true);
      await session.close();
    } finally {
      await harness.close();
  ...
  it("exposes the validated runtime binding through Session", async () => {
    ...
    try {
      ...
      await expect(session.switchRuntime("missing-runtime")).rejects.toThrow(
        /missing-runtime/,
      );
  ...
  it("reports global MCP authorization from the persisted v2 credential store", async () => {
    ...
    try {
      ...
      await externalOAuth.invalidate(
        "oauth-authorized",
        authorizedUrl,
        "tokens",
      );
  ...
  it("resumes a fallback session and keeps mutations in its owning home", async () => {
    ...
    const split = createKimiHarnessV2({
      homeDir,
      sessionHomeDir,
      identity: TEST_IDENTITY,
    });
    try {
      ...
      expect(summary?.sessionDir.startsWith(join(homeDir, "sessions"))).toBe(
        true,
      );
      await resumed.close();
    } finally {
  ...
  it("surfaces a missing Git Bash probe failure during ensureConfigFile on Windows", async () => {
    ...
    try {
      ...
      const harness = createKimiHarnessV2({ homeDir, identity: TEST_IDENTITY });
      try {
        await expect(harness.ensureConfigFile()).rejects.toBeInstanceOf(
          HostProcessError,
        );
...
`,
    ...
    try {
      ...
      const titlePromise = client.generateSessionTitle({
        id: "ses_title_race",
      });
      await fetchStarted;
      ...
      const order: string[] = [];
      const resumePromise = client
        .resumeSession({ id: "ses_title_race" })
        .then((summary) => {
          order.push("resumed");
          return summary;
        });
  ...
  it("rejects one of two concurrent creates with the same explicit session id", async () => {
    ...
    try {
      ...
      await expect(
        harness.resumeSession({ id: "ses_same_id" }),
  ...
  it("deleteSession removes a session and rejects a missing id with session_not_found", async () => {
    ...
    try {
      ...
      await expect(
        harness.resumeSession({ id: session.id }),
...
describe("removeProviderFromConfig", () => {
  ...
  it("drops the secondary_model section when its default model dangles", () => {
    ...
    expect(
      removeProviderFromConfig(config, "b").secondaryModel,
    ).toBeUndefined();
    ...
    expect(
      removeProviderFromConfig(legacy, "b").secondaryModel,
    ).toBeUndefined();
  });
