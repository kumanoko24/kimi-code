...
export class SDKRpcClientV2 extends SDKRpcClientBase {
  ...
  override async listSessionsPage(input: ListSessionsOptions = {}): Promise<SessionSummaryPage> {
    ...
    for (;;) {
      ...
      for (const item of page.items) {
        const summary = await this.mapIndexSummary(item, workspacesById);
  ...
  private async mapIndexSummary(
    ...
    const location = await this.engineAccessor.get(ISessionIndex).locate(item.id);
