...
### M22 — MCP OAuth callback upstream sync and isolated kiminn deployment (PASS)

Intent:

- merge current `MoonshotAI/main` at `102984aa6` into the fork without mutating upstream;
- accept upstream's MCP OAuth cancellation fix and its patch changeset without altering fork-owned 2234 behavior;
- rebuild and deploy only isolated `kiminn`, then prove its real parent, exact-bound subagent, resume, storage, and localhost-2234 lifecycle.

Recovery boundary: revert merge commit `4fdd39acb` and restore `/Users/noelbao/.kiminn/bin/kimi`, config, TUI config, receipt, and wrapper from `/Users/noelbao/.kiminn/backups/cumi-102984aa6-20260814-0040/`. Canonical Kimi and the gateway require no rollback.

Evidence:

- fresh fetch advanced upstream by one commit from `1414d4602` to `102984aa660d752ba8dd7d1aba155575f32affe2`; merge commit `4fdd39acb01f314e01fff7135ac2e14c034f44c2` applied without conflicts;
- the new real localhost OAuth callback and service-cancellation tests pass `3/3`; agent-core typecheck passes. The monorepo passes 1,107 test files with `18,519` tests, three expected failures, 80 skips, and two todos; full build and typecheck pass, while lint reports zero errors and 3,012 existing warnings;
- native SEA build, injection, ad-hoc signing, signature verification, native smoke, installed `kiminn doctor`, provider listing, and config parsing pass. Installed `/Users/noelbao/.kiminn/bin/kimi` reports `0.36.0` and matches the build at SHA-256 `3dab087f40bdc4a81a8673e970fc0af1451960bc81481e5a570151dc619a8401`;
- installed session `session_8ad56c88-bd65-45ea-9681-669339f4923e` returned exact `TASKER_102984_OK` from `gpt-coder-tasker`, exact parent `KIMINN_102984_RBV_OK`, and exact resumed `KIMINN_102984_RESUME_OK`. Wire facts show three parent requests on OpenAI Responses Sol/xhigh/272000 and one child request on OpenAI Responses Luna/max/272000; both parent turns and the child turn ended `completed`;
- the session exists only under canonical `~/.kimi-code/sessions`, preserving split-home ownership. Live gateway telemetry for the same affinity reports idle state, zero in-flight requests, safe account label `codex-leo`, and `4/4` HTTP 200 successes with usage observed for all four requests; `/dashboard` returned HTTP 200;
- isolated config, TUI config, and wrapper hashes stayed `83e076e6…`, `09d7953c…`, and `6729dac0…`. Canonical Kimi remained byte-identical at binary SHA-256 `d711c059…` and config SHA-256 `15ca0bd8…`;
- `/Users/noelbao/.kiminn/install-receipt.toml` records source `4fdd39acb`, upstream `102984aa6`, binary/config hashes, RBV session, split homes, and the rollback directory.

### M23 — upstream 0.36.1 sync and isolated kiminn deployment (PASS)

Intent:

- merge current `MoonshotAI/main` at `d96cd0377` into the fork without mutating upstream;
- integrate upstream 0.36.1, plugin/capability routes, task metadata, fork-at-turn, TUI safety, and goal-length fixes while preserving fork-owned split-home and exact 2234 model bindings;
- rebuild and deploy only isolated `kiminn`, then prove its real parent, exact-bound subagent, resume, storage, and localhost-2234 lifecycle.

Recovery boundary: revert merge commit `d431d3d85` and restore `/Users/noelbao/.kiminn/bin/kimi`, config, TUI config, receipt, and wrapper from `/Users/noelbao/.kiminn/backups/cumi-d96cd0377-20260815-0308/`. Canonical Kimi and the gateway require no rollback.

Evidence:

- fresh fetch advanced upstream by 14 commits from `102984aa6` to `d96cd037702637305422222e985139e51ff83c8c`, including release tag `@moonshot-ai/kimi-code@0.36.1`; merge commit `d431d3d85c431137fc25098e29d5386817180747` contains two resolved textual conflicts;
- the v2 session-lifecycle resolution combines upstream turn slicing and truncated-agent pruning with fork-owned source/target persistence scopes; the Node SDK test resolution retains both upstream cold-session checks and fork-owned separate auth/session-home contracts;
- focused v2 lifecycle tests pass `71/71`, Node SDK v2 tests pass `35/35`, and affected TUI tests pass `173/173`; v2 import-boundary guard and focused package typechecks pass. The monorepo passes 1,112 test files with `18,616` tests, three expected failures, 81 skips, and two todos; full build and serialized typecheck pass, while lint reports zero errors and 3,022 warnings;
- native SEA build, injection, ad-hoc signing, signature verification, native smoke, installed `kiminn doctor`, provider listing, and config parsing pass. Installed `/Users/noelbao/.kiminn/bin/kimi` reports `0.36.1` and matches the build at SHA-256 `69bb71d56207f82b0a078e0f505dbd6f77b05e98ec13c7c95200e759737c7825`;
- installed session `session_2d74a3a3-be0f-4ce0-b401-895532b8bd28` returned exact `TASKER_D96CD_OK` from `gpt-coder-tasker`, exact parent `KIMINN_D96CD_RBV_OK`, and exact resumed `KIMINN_D96CD_RESUME_OK`. Wire facts show three parent requests on OpenAI Responses Sol/xhigh/272000 and one child request on OpenAI Responses Luna/max/272000; both parent turns and the child turn ended `completed`;
- the session exists only under canonical `~/.kimi-code/sessions`, preserving split-home ownership. Live gateway telemetry for the same affinity reports idle state, zero in-flight requests, safe account label `codex-leo`, fresh quota, and `4/4` HTTP 200 terminal successes with usage observed for all four requests; `/dashboard` returned HTTP 200;
- isolated config, TUI config, and wrapper hashes stayed `83e076e6…`, `09d7953c…`, and `6729dac0…`. Canonical Kimi remained byte-identical at binary SHA-256 `d711c059…` and config SHA-256 `15ca0bd8…`;
- `/Users/noelbao/.kiminn/install-receipt.toml` records source `d431d3d85`, upstream `d96cd0377`, binary/config hashes, RBV session, split homes, and the rollback directory.

### M24 — upstream runtime-binding and multi-skill sync with isolated kiminn deployment (PASS)

Intent:

- merge current `MoonshotAI/main` at `44a6c70e6` into the fork without mutating upstream;
- integrate upstream session runtime binding, token-ledger persistence, fork-at-turn, and multi-skill activation while preserving fork-owned split-home storage and exact localhost-2234 model contracts;
- rebuild and deploy only isolated `kiminn`, then prove its real parent, exact-bound subagent, resume, storage, and localhost-2234 request lifecycle.

Recovery boundary: revert merge commit `ed411f8c6`, retain test-only cleanup commit `4fbd53a99` only if desired, and restore `/Users/noelbao/.kiminn/bin/kimi`, config, TUI config, receipt, and wrapper from `/Users/noelbao/.kiminn/backups/cumi-44a6c70e6-20260817-0634/`. Canonical Kimi and the gateway require no rollback.

Evidence:

- fresh fetch advanced upstream by six commits from `d96cd0377` to `44a6c70e66762ea9e122f8dceae16dc759086a7c`; merge commit `ed411f8c6` combines upstream runtime ownership with fork-owned `ISessionIndex.locate` and `sessionsScope` storage ownership. Commit `4fbd53a99` only makes VS Code integration cleanup tolerate asynchronous query-store teardown;
- focused v2 tests pass `141/141`, Node SDK tests pass `36/36`, the v2 import-boundary guard passes `1214/1214`, and isolated Kaos and VS Code integration reruns pass `17/17` and `48/48`. The full monorepo rerun passes 1,124 test files with `18,767` tests, three expected failures, 81 skips, and two todos; full build and serialized typecheck pass, while lint reports zero errors and 3,095 warnings;
- native SEA build, injection, ad-hoc signing, signature verification, native smoke, installed `kiminn doctor`, and provider listing pass. Installed `/Users/noelbao/.kiminn/bin/kimi` reports `0.36.1` and matches the build at SHA-256 `41d58bc0658aef0d524be7433282d1b886ff08a8fe631afa137cdd49b96d8603`;
- installed session `session_0db46f15-633b-4a13-9bf2-643b846270a6` returned exact child `TASKER_44A6C_OK`, exact parent `KIMINN_44A6C_RBV_OK`, and exact resumed `KIMINN_44A6C_RESUME_OK`. Wire facts show three parent requests on OpenAI Responses Sol/xhigh/272000 and one child request on OpenAI Responses Luna/max/272000; both parent turns and the child turn ended `completed`;
- the session exists under canonical `~/.kimi-code/sessions` and not isolated `~/.kiminn/sessions`, preserving split-home ownership. Gateway log affinity `session:9c88aee8…` records one new assignment and three sticky hits on safe label `codex-thx`; all `4/4` requests ended HTTP 200 with `response_completed` and observed usage;
- isolated config, TUI config, and wrapper hashes stayed `83e076e6…`, `09d7953c…`, and `6729dac0…`. The pre-existing canonical Kimi was not touched during this task and remained byte-identical to its fresh pre-deployment guard: binary SHA-256 `53b8a5d9…` and config SHA-256 `15ca0bd8…`;
- `/Users/noelbao/.kiminn/install-receipt.toml` records source `4fbd53a99`, upstream `44a6c70e6`, current binary/config/gateway hashes, RBV session, split homes, and the rollback directory.

### M25 — upstream 0.40.1 sync with isolated kiminn deployment (PASS)

Intent:

- merge current `MoonshotAI/main` at `052e98ec1` into the fork without mutating upstream;
- preserve native OpenAI Responses compaction, provider observability, media fallback, split auth/session homes, and exact file-defined subagent bindings across the upstream v2 architecture changes;
- rebuild and deploy only isolated `kiminn`, then prove the real parent, exact-bound child, resume, storage ownership, and localhost-2234 request lifecycle.

Recovery boundary: revert merge commit `ecaf55b50` and restore `/Users/noelbao/.kiminn/bin/kimi` plus its receipt from `/Users/noelbao/.kiminn/backups/cumi-052e98ec1-20260904-0724/`. Canonical Kimi, isolated configuration/wrapper, credentials, and the gateway require no rollback.

Evidence:

- fresh fetch advanced upstream from `44a6c70e6` to `052e98ec17ac8d931873ef892fb0e1912aa401e0`; merge commit `ecaf55b508fa50f486eb21a0de0b5f4b38e641a1` resolves 36 textual conflicts and reapplies the fork behavior at the new v2 service, projector, session lifecycle, and TUI boundaries;
- focused fork regressions passed, root lint passed with zero errors, the full root typecheck and 23-workspace build passed, native SEA injection/signature verification/smoke passed, and package lint passed. Sherif reported only two upstream workspace-glob warnings for directories that no longer contain package manifests;
- the clean full-suite run with four workers passed 1,140 files and 20,580 tests, with three expected failures, 81 skips, and two todos. Higher worker counts exposed only scheduler-sensitive existing tests: the affected klient cases passed in 35–40 ms alone, and the filesystem-watch file passed three consecutive isolated runs, 30/30 tests;
- installed `/Users/noelbao/.kiminn/bin/kimi` reports `0.40.1`, matches the built darwin-arm64 artifact at SHA-256 `344b649aebcad55b6b302d02fdf8865ee3f81e4ed42c434dc70a5de1a7a07db4`, and passes `kiminn doctor`; the rollback binary remains executable and reports `0.36.1`;
- installed session `session_ed67181d-6ee2-435c-b30b-fcf12d882e37` returned exact child `CHILD_LUNA_MAX_OK`, exact parent `PARENT_SOL_XHIGH_OK CHILD_LUNA_MAX_OK`, and exact resumed `RESUME_OK`. Session logs and wire events record the parent on OpenAI Responses Sol/xhigh and the child on OpenAI Responses Luna/max with a 272000-token request cap;
- the session exists only under canonical `~/.kimi-code/sessions`, not isolated `~/.kiminn/sessions`. Gateway affinity `3b1f552c…` records one new assignment and three sticky hits; all `4/4` requests ended HTTP 200 with `response_completed`, observed usage, idle final state, and zero in-flight requests;
