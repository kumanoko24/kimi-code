...
| `model_preference` | no | Symbolic spawn default: `primary` binds the caller's current model and thinking level; `secondary` uses the configured subagent model-pool default, or inherits the caller when no pool is active. Mutually exclusive with `model` |
| `model` | no | Exact model alias for newly spawned subagents. Requires the experimental agent-profile model-binding flag. Mutually exclusive with `model_preference` |
| `thinking_effort` | no | Exact thinking effort paired with `model`; invalid without `model` |
...
Unknown fields are ignored, so newer files stay readable by older versions. OpenCode's `mode` is ignored; Claude Code's `model` is recognized as the opt-in exact binding above. The comma-separated `tools` form keeps Claude Code-style agent files loadable, and a missing `name` falls back to the file name so OpenCode-style files load too — a minimal file with `description` and a body works across tools.

These fields affect only newly spawned subagents; resumed subagents keep their existing binding. An explicit `model` passed by the `Agent` or `AgentSwarm` tool wins over the profile fields. A forced `[secondary_model]` configuration wins over both and rejects explicit choices. Exact `model` / `thinking_effort` binding is inert unless `KIMI_CODE_EXPERIMENTAL_AGENT_PROFILE_MODEL_BINDING=1` or the master `KIMI_CODE_EXPERIMENTAL_FLAG=1` is enabled.
