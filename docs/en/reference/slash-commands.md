...
| `/compact [<instruction>]` | — | Compact the current conversation context to free up token usage. Providers configured for native compaction use their compact endpoint; others use text compaction. An optional instruction hints what to preserve | No |
...
| `/usage` | — | Show token usage, context consumption, quota information, and configured gateway account/cache aggregates | Yes |
