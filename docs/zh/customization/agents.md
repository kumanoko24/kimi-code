...
| `model_preference` | 否 | 符号式启动默认值：`primary` 绑定调用方当前模型与 thinking level；`secondary` 使用已配置的 subagent 模型池默认值，未启用模型池时继承调用方。不可与 `model` 同时设置 |
| `model` | 否 | 新启动 subagent 使用的精确模型 alias。需启用实验性 agent-profile model binding flag。不可与 `model_preference` 同时设置 |
| `thinking_effort` | 否 | 与 `model` 配套的精确 thinking effort；未设置 `model` 时非法 |
...
未知字段会被忽略，新版本写的文件在旧版本上仍可读取。OpenCode 的 `mode` 会被忽略；Claude Code 的 `model` 会被识别为上述 opt-in 精确绑定。加上 `tools` 的逗号分隔写法和 `name` 缺省回退到文件名，Claude Code 与 OpenCode 风格的 Agent 文件一般可直接加载 —— 只含 `description` 和正文的最小文件可跨工具通用。

这些字段只影响新启动的 subagent；恢复的 subagent 保留既有绑定。`Agent` 或 `AgentSwarm` 工具显式传入的 `model` 优先于 profile 字段；强制 `[secondary_model]` 配置优先于两者，并拒绝显式选择。精确 `model` / `thinking_effort` 只有在启用 `KIMI_CODE_EXPERIMENTAL_AGENT_PROFILE_MODEL_BINDING=1` 或 master `KIMI_CODE_EXPERIMENTAL_FLAG=1` 时生效。
